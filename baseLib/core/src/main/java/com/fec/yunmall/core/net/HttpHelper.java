...
public class HttpHelper {
    ...
    private static SparseArray<HttpHelper> sRetrofitManager = new SparseArray<>(BaseHost.TYPE_COUNT);
...
/*
   1. noCache 不使用缓存，全部走网络
    ...
    private HttpHelper(int hostType){
        ...
        //缓存
        File cacheFile = new File(CoreApplication.getApplication().getCacheDir(), "cache");
    ...
    public static Retrofit getDefault() {
        int hostType = (int)SPUtil.get(CoreApplication.getApplication(),BaseHost.SELECT_SERVER,1);
        HttpHelper retrofitManager = sRetrofitManager.get(hostType);
        if (retrofitManager == null) {
            retrofitManager = new HttpHelper(hostType);
            sRetrofitManager.put(hostType, retrofitManager);
